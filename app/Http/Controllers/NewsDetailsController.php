    ...
    public function index(Request $request, $newscategory_id = '', $id = '')
{
    ...
    if ($request->ajax()) {
        ...
        if ($newscategory_id != '') {
            $data = NewsDetails::with('newscategory')
                ->where('newscategory_id', $newscategory_id)
                ->when(auth()->user()->hasRole('viewer'), function ($query) use ($today) {
                    return $query->where(function ($query) use ($today) {
                        $query->whereNull('enddate') // Include if enddate is null
                              ->orWhere(DB::raw("STR_TO_DATE(enddate, '%d/%m/%Y')"), '>=', $today);
                    });
                })
                ->orderBy('id', 'desc')
                ->get();
